...
def map_ins_plot1(df, state):
    ...
    with col1:
        ...
        fig=px.bar(mip_g, x="Transaction_amount", y="Districts", title=f"{state} Districts Transaction Amount", color_discrete_sequence=px.colors.sequential.Blues_r)
        st.plotly_chart(fig)

    with col2:
        st.subheader(":blue[Districts Transaction Amount]")
        fig=px.bar(mip_g, x="Transaction_count", y="Districts", title=f"{state} Districts Transaction Count", color_discrete_sequence=px.colors.sequential.Aggrnyl_r)
        st.plotly_chart(fig)   
...
def top_ins1_bar(df,year, quarter):
    tia=df[(df["Years"]==year) & (df["Quarters"]==quarter)]
...
col1, col2=st.columns(2)
with col1:
    st.image(Image.open(r"248807513-f43c154d-50db-4d22-aaa0-02e79d3034d3.png"), width=200)
    ...
    with tab3:
        ...
        if button3=="Top Insurance":
            ...
            with col2:
                ...
                quarter=st.selectbox("**Select the quarter**    ", (top_insurance["Quarters"].unique()))    
